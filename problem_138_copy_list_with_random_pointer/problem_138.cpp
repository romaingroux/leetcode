...
public:
    ...
    /**
     * Copies a singly linked list of nodes containing a pointer to a
     * random other node in the list.
    ...
    Node* copyRandomList(Node* head) {
        ...
        // finish construction by adding next and random
        current = head ;
        while(current)
        {   auto iter = map.find(current) ;
            if(current->next)
            {   iter->second->next = map[current->next] ; }
            if(current->random)
            {   iter->second->random = map[current->random] ; }
            current = current->next ;
        }
